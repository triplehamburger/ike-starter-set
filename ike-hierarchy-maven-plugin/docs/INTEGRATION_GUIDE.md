...
Set the version once, in a property, so both declarations move together and no adopting build
hardcodes a snapshot:

```xml
<properties>
    <!-- The ike-hierarchy release you are adopting. -->
    <ike-hierarchy.version>REPLACE-ME</ike-hierarchy.version>
</properties>
```
...
<build>
    <plugins>
        <plugin>
            ...
            <version>${ike-hierarchy.version}</version>
            <executions>
                <execution>
