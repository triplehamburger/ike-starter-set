...
```bash
mvn ike-hierarchy:add-chapter -pl ike-doc -Dfile=ike-doc/src/docs/asciidoc/my-draft.adoc -Dparent=ike-guide -Dorder=250
```

Run it from the project root. `-pl ike-doc` is required — the plugin is declared only in
`ike-doc/pom.xml`, and without it Maven reports `No plugin found for prefix 'ike-hierarchy'`.
`-Dfile` still resolves against the project root, not against `ike-doc`.
...
```bash
mvn ike-hierarchy:decompose -pl ike-doc -Dfile=ike-doc/src/docs/asciidoc/ike-guide.adoc              # show the plan
mvn ike-hierarchy:decompose -pl ike-doc -Dfile=ike-doc/src/docs/asciidoc/ike-guide.adoc -Dwrite=true # apply it
```
