...
/**
 * The "from" section — the CQL keyword {@code from} as a concept, filed under
 ...
 * {@code [[term-from]]}): a Query Clause of the Query &amp; Control Flow family, Komet status
 * "Not yet in Komet".
 */
final class From {
    ...
    static void compose(KnowledgeSet set) {
        ...
        UUID concept = set.uuidFor("from (CQL)");
        UUID fullyQualifiedName = set.uuidFor("from (CQL) fully qualified name description");
        UUID regularName = set.uuidFor("from (CQL) regular name description");
        UUID definition = set.uuidFor("from (CQL) definition description");

        set.concept("from (CQL)", PublicIds.of(concept)).at(inception)
                ...
                .semanticOn(PublicIds.of(fullyQualifiedName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("from (CQL) fully qualified name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(regularName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("from (CQL) regular name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(definition), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("from (CQL) definition US dialect")), IkeTerm.PREFERRED)  // dialect pref
                ;
