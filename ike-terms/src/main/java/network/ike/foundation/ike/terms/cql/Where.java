...
/**
 * The "where" section — the CQL keyword {@code where} as a concept, filed under
 ...
 * {@code [[term-where]]}): a Query Clause of the Query &amp; Control Flow family, Komet status
 * "Not yet in Komet".
 */
final class Where {
    ...
    static void compose(KnowledgeSet set) {
        ...
        UUID concept = set.uuidFor("where (CQL)");
        UUID fullyQualifiedName = set.uuidFor("where (CQL) fully qualified name description");
        UUID regularName = set.uuidFor("where (CQL) regular name description");
        UUID definition = set.uuidFor("where (CQL) definition description");

        set.concept("where (CQL)", PublicIds.of(concept)).at(inception)
                ...
                .semanticOn(PublicIds.of(fullyQualifiedName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("where (CQL) fully qualified name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(regularName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("where (CQL) regular name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(definition), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("where (CQL) definition US dialect")), IkeTerm.PREFERRED)  // dialect pref
                ;
