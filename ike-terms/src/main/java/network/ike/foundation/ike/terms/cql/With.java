...
/**
 * The "with" section — the CQL keyword {@code with} as a concept, filed under
 ...
 * {@code [[term-with]]}): a Query Clause of the Query &amp; Control Flow family, Komet status
 * "Not yet in Komet".
 */
final class With {
    ...
    static void compose(KnowledgeSet set) {
        ...
        UUID concept = set.uuidFor("with (CQL)");
        UUID fullyQualifiedName = set.uuidFor("with (CQL) fully qualified name description");
        UUID regularName = set.uuidFor("with (CQL) regular name description");
        UUID definition = set.uuidFor("with (CQL) definition description");

        set.concept("with (CQL)", PublicIds.of(concept)).at(inception)
                ...
                .semanticOn(PublicIds.of(fullyQualifiedName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("with (CQL) fully qualified name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(regularName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("with (CQL) regular name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(definition), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("with (CQL) definition US dialect")), IkeTerm.PREFERRED)  // dialect pref
                ;
