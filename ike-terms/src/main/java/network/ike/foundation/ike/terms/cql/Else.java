...
/**
 * The "else" section — the CQL keyword {@code else} as a concept, filed under
 ...
 * {@code [[term-else]]}): a Conditional Expression of the Query &amp; Control Flow family,
...
final class Else {
    ...
    static void compose(KnowledgeSet set) {
        ...
        UUID concept = set.uuidFor("else (CQL)");
        UUID fullyQualifiedName = set.uuidFor("else (CQL) fully qualified name description");
        UUID regularName = set.uuidFor("else (CQL) regular name description");
        UUID definition = set.uuidFor("else (CQL) definition description");

        set.concept("else (CQL)", PublicIds.of(concept)).at(inception)
                ...
                .semanticOn(PublicIds.of(fullyQualifiedName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("else (CQL) fully qualified name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(regularName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("else (CQL) regular name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(definition), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("else (CQL) definition US dialect")), IkeTerm.PREFERRED)  // dialect pref
                ;
