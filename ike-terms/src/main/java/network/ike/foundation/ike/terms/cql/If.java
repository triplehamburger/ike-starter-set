...
/**
 * The "if" section — the CQL keyword {@code if} as a concept, filed under
 ...
 * {@code [[term-if]]}): a Conditional Expression of the Query &amp; Control Flow family, Komet
...
final class If {
    ...
    static void compose(KnowledgeSet set) {
        ...
        UUID concept = set.uuidFor("if (CQL)");
        UUID fullyQualifiedName = set.uuidFor("if (CQL) fully qualified name description");
        UUID regularName = set.uuidFor("if (CQL) regular name description");
        UUID definition = set.uuidFor("if (CQL) definition description");

        set.concept("if (CQL)", PublicIds.of(concept)).at(inception)
                ...
                .semanticOn(PublicIds.of(fullyQualifiedName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("if (CQL) fully qualified name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(regularName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("if (CQL) regular name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(definition), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("if (CQL) definition US dialect")), IkeTerm.PREFERRED)  // dialect pref
                ;
