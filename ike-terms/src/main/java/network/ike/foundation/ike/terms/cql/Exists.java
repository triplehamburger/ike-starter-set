...
/**
 * The "exists" section — the CQL keyword {@code exists} as a concept, filed under
 ...
 * {@code [[term-exists]]}): a Logical Operator of the Core Operators family, Komet status "Not
 * yet in Komet".
 */
final class Exists {
    ...
    static void compose(KnowledgeSet set) {
        ...
        UUID concept = set.uuidFor("exists (CQL)");
        UUID fullyQualifiedName = set.uuidFor("exists (CQL) fully qualified name description");
        UUID regularName = set.uuidFor("exists (CQL) regular name description");
        UUID definition = set.uuidFor("exists (CQL) definition description");

        set.concept("exists (CQL)", PublicIds.of(concept)).at(inception)
                ...
                .semanticOn(PublicIds.of(fullyQualifiedName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("exists (CQL) fully qualified name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(regularName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("exists (CQL) regular name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(definition), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("exists (CQL) definition US dialect")), IkeTerm.PREFERRED)  // dialect pref
                ;
