...
/**
 * The "overlaps" section — the CQL keyword {@code overlaps} as a concept, filed under
 ...
 * {@code [[term-overlaps]]}): a Timing Operator of the Data &amp; Timing Operators family,
 * Komet status "Related concept in Komet".
 */
final class Overlaps {
    ...
    static void compose(KnowledgeSet set) {
        ...
        UUID concept = set.uuidFor("overlaps (CQL)");
        UUID fullyQualifiedName = set.uuidFor("overlaps (CQL) fully qualified name description");
        UUID regularName = set.uuidFor("overlaps (CQL) regular name description");
        UUID definition = set.uuidFor("overlaps (CQL) definition description");

        set.concept("overlaps (CQL)", PublicIds.of(concept)).at(inception)
                ...
                .semanticOn(PublicIds.of(fullyQualifiedName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("overlaps (CQL) fully qualified name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(regularName), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("overlaps (CQL) regular name US dialect")), IkeTerm.PREFERRED)  // dialect pref
                .semanticOn(PublicIds.of(definition), IkeTerm.US_DIALECT_PATTERN, PublicIds.of(set.uuidFor("overlaps (CQL) definition US dialect")), IkeTerm.PREFERRED)  // dialect pref
                ;
